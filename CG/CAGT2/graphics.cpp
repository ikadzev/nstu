...
draw_t getDrawType() {
	...
	switch(i) {
		...
		case 6: return triangulation;
		case 7: cout << "Nuh-uh" << endl; return draw_t::null;
		case 8: return render;
...
void getCoords(TableInfo& ti, draw_t type) {
	...
	if ((type == cda) || (type == triangle) || (type == bresenham)) {
		...
		cin >> y;
		ti.dots.push_back(Point{x, y});
	} else if (type == polygon || type == triangulation) {
		Point first_dot;
		cout << "First dot (x, y);" << endl;
		cin >> first_dot.x;
		cin >> first_dot.y;
...
void triang(SDL_Renderer* ren, TableInfo& ti, int wall_color) {
	for (Point p : ti.dots) writeRect(ren, ti, p.x, TABLE_SIZE - p.y + 1, wall_color);
	vector<Line> all_lines;
	for (int i = 0; i < ti.dots.size(); i++) for (int j = i + 1; j < ti.dots.size(); j++) all_lines.push_back(Line{ti.dots[i], ti.dots[j]});
	
	sort(all_lines.begin(), all_lines.end(), comp);
	
	vector<Line> done_lines;
	for (Line& l1 : all_lines) {
		bool flag = true;
		for (Line& l2 : done_lines) {
		...
		if (flag) {
			done_lines.push_back(l1);
			writeLineCDA(ti, ren, l1.s.x, l1.s.y, l1.e.x, l1.e.y, wall_color);
		}
	}
}
...
bool intersection(Line& l1, Line& l2) {
	double x1 = l1.s.x, y1 = l1.s.y;
	double x2 = l1.e.x, y2 = l1.e.y;
	double x3 = l2.s.x, y3 = l2.s.y;
	double x4 = l2.e.x, y4 = l2.e.y;
...
// Compare function for sorting lines
bool comp(Line& l1, Line& l2) { 
	int dx1 = l1.e.x - l1.s.x;
	int dx2 = l2.e.x - l2.s.x;
	int dy1 = l1.e.y - l1.s.y;
	int dy2 = l2.e.y - l2.s.y;

	int length1 = sqrt(dx1 * dx1 + dy1 * dy1);
	int length2 = sqrt(dx2 * dx2 + dy2 * dy2);
